...
export const repoName = readEnv("REPO_NAME") ?? readEnv("NEXT_PUBLIC_REPO_NAME") ?? parsedFromUrl?.name ?? githubParts?.name ?? "biyori";

export const repoTitle = readEnv("REPO_TITLE") ?? readEnv("NEXT_PUBLIC_REPO_TITLE") ?? "Biyori";

export const repoUrl = readEnv("REPO_URL") ?? readEnv("NEXT_PUBLIC_REPO_URL") ?? `https://github.com/${repoOwner}/${repoName}`;

export const repoBranch = readEnv("REPO_BRANCH") ?? readEnv("NEXT_PUBLIC_REPO_BRANCH") ?? readEnv("GITHUB_REF_NAME") ?? "master";
...
export const siteUrl = readEnv("NEXT_PUBLIC_URL") ?? (process.env.GITHUB_PAGES === "true" ? `https://${repoOwner.toLowerCase()}.github.io/${repoName}` : "http://localhost:3000");

export const brandColor = "#3d5aa3";
