...
export function parseReleaseVersion(version: string): ParsedReleaseVersion | null {
	const match = version.match(/(\d+)\.(\d+)\.(\d+)(?:-(rc|a|alpha)(?:\.(\d+))?)?/i);
 ...
 */
export function listVisibleReleaseChannels(releases: Partial<Record<ReleaseChannel, LatestRelease | null>>): ReleaseChannel[] {
 ...
 */
export function resolveReleaseChannel(release: { tag_name: string; name?: string | null; prerelease?: boolean }): ReleaseChannel | null {
...
export type DownloadKind = "windows-setup" | "macos-dmg" | "macos-zip" | "macos-pkg" | "linux-appimage" | "linux-deb" | "linux-rpm" | "linux-flatpak" | "other";
...
export function listUserDownloads(assets: ReleaseAsset[]): ReleaseAsset[] {
	const order: DownloadKind[] = ["windows-setup", "macos-dmg", "macos-pkg", "macos-zip", "linux-appimage", "linux-deb", "linux-rpm", "linux-flatpak"];
...
export function groupDownloadsByPlatform(assets: ReleaseAsset[]): Record<DownloadPlatform, ReleaseAsset[]> {
...
export function pickPreferredDownload(assets: ReleaseAsset[], preferredArch?: CpuArch | null): ReleaseAsset | undefined {
...
export function pickPrimaryDownload(assets: ReleaseAsset[]): ReleaseAsset | undefined {
	...
	return groups.windows[0] ?? groups.macos[0] ?? groups.linux[0] ?? listUserDownloads(assets)[0];
}
