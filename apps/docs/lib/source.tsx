...
export const ogImageSchema = z
	.object({
		image: z.string().optional().describe("Public path, e.g. /images/features-trayview-compact.png"),
				...
				file(node, filePath) {
					...
					node.name = (
						<span className='inline-flex items-center gap-2'>
							<span>{node.name}</span>
							<span className='rounded-full border border-fd-border bg-fd-muted px-1.5 py-0.5 text-[10px] font-medium leading-none text-fd-muted-foreground'>{badge}</span>
						</span>
					);
