...
function pickDefaultChannel(releases: Partial<Record<ReleaseChannel, LatestRelease | null>>): ReleaseChannel {
...
export function ReleaseDownloadPanel({ releases }: ReleaseDownloadPanelProps) {
	...
	const availableChannels = useMemo(() => listVisibleReleaseChannels(releases), [releases]);

	const release = releases[channel] ?? null;
	const groups = useMemo(() => (release ? groupDownloadsByPlatform(release.assets) : null), [release]);
	...
	if (!release || !groups) {
		return (
			<aside className='relative rounded-2xl border bg-fd-background/85 p-6 backdrop-blur-md'>
				...
				<a href={getReleasesUrl()} target='_blank' rel='noreferrer' className={cn(buttonVariants({ variant: "secondary" }), "mt-5 gap-2 px-4 py-2")}>
					Browse releases
	...
	return (
		<aside className='relative rounded-2xl border bg-fd-background/85 p-6 backdrop-blur-md'>
			<div className='mb-4 flex items-center justify-between gap-3'>
								...
								return (
									<li key={item} role='option' aria-selected={selected}>
										<button
											...
											className={cn("flex w-full flex-col gap-0.5 px-3 py-2 text-left text-sm transition-colors hover:bg-fd-accent", selected && "bg-fd-accent/60")}>
											<span className='font-medium text-fd-foreground'>{RELEASE_CHANNEL_LABELS[item]}</span>
											{itemRelease ? <span className='text-xs text-fd-muted-foreground'>{itemRelease.tag_name}</span> : null}
										</button>
									</li>
								);
							})}
				...
				<span className='rounded-full border bg-fd-secondary px-2.5 py-1 text-xs font-medium'>{release.tag_name}</span>
			</div>
			...
			{notes.length > 0 ? <ReleaseNotesCollapse notes={notes} expanded={notesExpanded} onToggle={() => setNotesExpanded((value) => !value)} /> : null}

			<div role='tablist' aria-label='Download platform' className='mt-5 flex flex-wrap gap-1 rounded-xl border bg-fd-secondary/40 p-1'>
					...
					return (
						<button
							...
							className={cn(
								...
								selected ? "bg-fd-background text-fd-foreground shadow-sm" : "text-fd-muted-foreground hover:text-fd-foreground",
							)}>
							{item.label}
							{count === 0 ? <span className='ml-1 text-[10px] font-normal opacity-70'>soon</span> : null}
						</button>
					);
				})}
			</div>
			...
			{primary ? (
				<>
					<a href={primary.browser_download_url} className={cn(buttonVariants({ variant: "primary" }), "mt-4 w-full gap-2 px-4 py-2")}>
				...
				<div className='mt-4 rounded-xl border border-dashed bg-fd-secondary/30 px-4 py-5 text-sm text-fd-muted-foreground'>
					...
					<p className='mt-1.5 text-pretty'>When a {platformLabel} installer is published to GitHub Releases, it shows up here automatically.</p>
					<a href={release.html_url} target='_blank' rel='noreferrer' className='mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-fd-primary hover:underline'>
			...
			<a href={release.html_url} target='_blank' rel='noreferrer' className='mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-fd-primary hover:underline'>
				View on GitHub
				<ArrowUpRightIcon className='size-3.5' />
			</a>
		</aside>
...
function ReleaseNotesCollapse({ notes, expanded, onToggle }: ReleaseNotesCollapseProps) {
	...
	if (expanded) {
		return (
			<div className='mt-4'>
				<div className='max-h-[min(32rem,70vh)] overflow-hidden transition-[max-height] duration-300 ease-out'>{list}</div>
				<button type='button' onClick={onToggle} className='mt-2 text-xs font-medium text-fd-primary transition-colors hover:text-fd-foreground'>
					Collapse
				</button>
			</div>
		);
...
function DownloadRow({ asset }: { asset: ReleaseAsset }) {
	...
	return (
		<li>
			<a href={asset.browser_download_url} className='group flex items-center justify-between gap-3 rounded-xl border px-3 py-2.5 text-sm transition-colors hover:bg-fd-accent'>
				<span className='min-w-0'>
					<span className='block font-medium'>{label.title}</span>
					<span className='block truncate text-xs text-fd-muted-foreground'>{label.description}</span>
				</span>
