			...
			<TabsContent value="general" className="pt-4">
				<FieldGroup>
					<FormCheckbox
						...
						id={enableId}
						label="Enable media recognition"
					/>
					<FieldSet className="rounded-md border p-3">
						...
						<FormCheckbox
							control={form.control}
							name="ignoreOutsideLibrary"
							id={ignoreId}
							label="Ignore files outside of library folders"
						/>
						<FormCheckbox
							control={form.control}
							name="ignoreOutOfRangeEpisode"
							id={rangeId}
							label="Ignore if episode number is greater than next episode"
						/>
					</FieldSet>
					<FieldSet className="rounded-md border p-3">
						<FieldLegend variant="label" className="text-muted-foreground">
						...
						<FormCheckbox
							control={form.control}
							name="notifyOnRecognized"
							id={notifyOkId}
							label="Notify me"
						/>
						<FormCheckbox
							control={form.control}
							name="goToNowPlayingOnRecognized"
							id={gotoOkId}
							label="Go to Now Playing"
						/>
						<p className="text-sm font-medium">
							When an episode is not recognized:
						</p>
						<FormCheckbox
							control={form.control}
							name="notifyOnUnrecognized"
							id={notifyBadId}
							label="Notify me"
						/>
						<FormCheckbox
							control={form.control}
							name="goToNowPlayingOnUnrecognized"
							id={gotoBadId}
							label="Go to Now Playing"
						/>
					</FieldSet>
					<FieldSet className="rounded-md border p-3">
						...
						<Field>
							...
							<FieldError
								errors={[form.formState.errors.recognitionDelaySeconds]}
							/>
						</Field>
						<FormCheckbox
							control={form.control}
							name="playerMustBeInFocus"
							id={focusId}
							label="Media player must be in focus"
						/>
						<FormCheckbox
							control={form.control}
							name="waitUntilPlayerExits"
							id={waitId}
							label="Wait for media player to close"
						/>
						<FormCheckbox
							control={form.control}
							name="askToConfirmUpdate"
							id={confirmId}
					...
					<FieldSet className="rounded-md border p-3">
						...
						<Field>
							<FieldLabel htmlFor={ignoredId}>
								Strip these from titles before matching:
							</FieldLabel>
							<Textarea
								id={ignoredId}
								className="min-h-20 font-mono text-xs"
								{...form.register("ignoredStrings")}
							/>
