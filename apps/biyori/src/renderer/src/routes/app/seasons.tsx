...
function SeasonsPage() {
	...
	const season =
		local.season ?? settings?.seasonsLastSeason ?? fallback.season;
	const seasonYear =
		local.seasonYear ?? settings?.seasonsLastYear ?? fallback.seasonYear;
	...
	const query = trpc.anilist.season.useQuery(
		{ season, seasonYear, forceRefresh: false },
		{ enabled: ready },
	);
...
function SeasonCard(props: {
	...
	const { item, viewAs, sortBy, listStatus, onOpen, onAdd, adding } =
		props;
			...
			<ContextMenuTrigger
				className={
					viewAs === "images"
						...
						: cn(
								"group flex w-full cursor-pointer gap-3 rounded-md border bg-card p-2 ring-1 ring-transparent",
				...
				{viewAs === "images" ? (
					...
					<>
						<div className="flex w-28 shrink-0 flex-col gap-1.5">
							...
							{inList ? (
								<span className="block truncate rounded-md border px-2 py-1 text-center text-xs text-muted-foreground">
									{listStatus}
								</span>
							) : (
								<Button
						...
						<div className="min-w-0 flex-1 text-left">
							<div
								className={cn(
									"mb-2 flex items-center gap-2 rounded-sm px-2 py-1.5 text-sm font-semibold",
									bar,
								)}
							>
