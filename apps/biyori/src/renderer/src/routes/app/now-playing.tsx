...
import {
  ...
  AlertDescription,
  AlertTitle,
} from "@/mainview/components/ui/alert";
...
import {
  ...
  ProgressLabel,
  ProgressValue,
} from "@/mainview/components/ui/progress";
...
type NowPlayingSnapshot = NonNullable<
  inferRouterOutputs<AppRouter>["media"]["nowPlaying"]
>;
...
function IdleNowPlaying() {
  ...
  const skipStatus = new Set(
    (listedQuery.data ?? [])
      .filter(
        (row) => row.status === "Completed" || row.status === "Dropped",
      )
      .map((row) => row.id),
  );
  ...
  if (continueWatching.length === 0) {
    return (
      <PlaceholderView
        ...
        description="Episodes you're currently watching will show up here."
      />
    );
  }
      ...
      <div className="mx-auto flex w-full flex-col gap-6 p-4 pb-10">
        <header className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            Continue from recent list updates.
          </p>
        </header>
...
function MatchedPlayback({ snapshot }: { snapshot: NowPlayingSnapshot }) {
  ...
  const detailQuery = trpc.anime.byId.useQuery(
    { id: match?.id ?? 0 },
    { enabled: Boolean(match?.id), staleTime: 60_000 },
  );
  if (!match || !media) {
    return null;
  }
  ...
  const progressValue =
    total != null && total > 0
      ...
      : 80; // Default progress value to 80% if total is not available
      ...
      <div className="relative h-44 w-full overflow-hidden bg-muted">
        {detail?.bannerUrl || match.bannerUrl ? (
          <AnimeCover
            id={match.id}
            kind="banner"
            sourceUrl={detail?.bannerUrl || match.bannerUrl}
            alt=""
        ...
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 bg-gradient-to-t from-background via-background/75 to-background/20"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-0 w-2/3 bg-gradient-to-l from-background/50 to-transparent"
        />
      </div>
      ...
      <div className="relative z-10 -mt-16 grid grid-cols-[14rem_1fr] items-start gap-x-4 gap-y-3 px-4">
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {airingStatus ? (
              <Badge variant="outline">{airingStatus}</Badge>
            ) : null}
            {snapshot.delayRemainingSeconds > 0 ? (
              <Badge variant="outline">
                Updating in {snapshot.delayRemainingSeconds}s
              </Badge>
            ) : null}
            {snapshot.pendingConfirm ? (
              <Badge>Confirm update</Badge>
            ) : null}
          </div>
          ...
          {progressValue != null ? (
            <Progress value={progressValue} className="w-full max-w-48">
              ...
              <ProgressValue>
                {() =>
                  total != null ? `${watched} / ${total}` : `${watched} / ?`
                }
              </ProgressValue>
            </Progress>
          ) : (
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              render={
                <a
                  href={`https://anilist.co/anime/${match.id}`}
                  target="_blank"
          ...
          {detailQuery.isLoading && !detail ? (
            ...
            <AnimeSeriesInfo
              anime={{
                alternativeTitles:
                  detail?.alternativeTitles ?? match.alternativeTitles ?? "",
                type: detail?.type ?? match.type ?? "",
                episodes: detail?.episodes ?? match.episodes,
                airingStatus:
                  detail?.airingStatus ?? match.airingStatus ?? "",
...
function UnrecognizedPlayback({
  ...
  const title =
    snapshot.parsed?.title ?? snapshot.media?.title ?? "Unknown title";
  const episode = snapshot.parsed?.episode;
  const group = snapshot.parsed?.group;
  ...
  return (
    <>
      <header className="flex flex-col gap-4 rounded-xl border bg-card p-4 sm:flex-row sm:items-start">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-sm text-muted-foreground">
              {formatNowPlayingLine(episode, group)}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-1.5">
            <Badge variant="destructive">Not recognized</Badge>
            {snapshot.media ? (
              <Badge variant="outline">{snapshot.media.player}</Badge>
            ) : null}
          </div>
        </div>
      </header>
      ...
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Biyori could not identify this episode against your list. Search
          AniList and add it, or check the filename.
        </AlertDescription>
        {searchQuery ? (
          <AlertAction>
            <Button
...
function buildContinueWatching(
  ...
  for (const row of rows) {
    if (
      row.animeId <= 0 ||
      row.episode <= 0 ||
      seen.has(row.animeId) ||
      skipAnimeIds.has(row.animeId)
    ) {
      continue;
    }
...
function formatNowPlayingLine(
  ...
  const episodePart =
    episode != null ? `Episode ${episode}` : "Episode unknown";
  if (group) {
