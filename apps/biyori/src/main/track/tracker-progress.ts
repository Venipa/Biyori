...
export function canApplyProgress(
	match: ProgressMatch,
	episode: number,
	settings: Pick<AppSettings, "ignoreOutOfRangeEpisode">,
): boolean {
	const progress =
		match.rewatching &&
		match.episodes > 0 &&
		match.episodesWatched >= match.episodes
			? 0
			: match.episodesWatched;
	if (episode <= progress) {
		return false;
	}
...
export function progressPayload(
	...
	return {
		...
		timesRewatched: completedRewatch
			? match.timesRewatched + 1
			: undefined,
		dateStarted:
			episode >= 1 && !match.dateStarted
