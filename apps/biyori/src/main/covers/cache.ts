...
export async function readCachedMediaImage(options: { db: DatabaseClient; animeId: number; kind: MediaImageKind }): Promise<CachedMediaImage | null> {
	const id = rowId(options.kind, options.animeId);
	const rows = await options.db.select().from(mediaCache).where(eq(mediaCache.id, id)).limit(1);
 ...
 */
export async function getOrFetchMediaImage(options: { db: DatabaseClient; animeId: number; kind: MediaImageKind; sourceUrl?: string }): Promise<CachedMediaImage> {
	const id = rowId(options.kind, options.animeId);
	const rows = await options.db.select().from(mediaCache).where(eq(mediaCache.id, id)).limit(1);
	const existing = rows[0];
	...
	if (existing?.mime && existing.fileName) {
		const file = join(cacheDir(), existing.fileName);
		const urlMatches = !sourceUrl || existing.sourceUrl === sourceUrl;
...
export async function getOrFetchCover(options: { db: DatabaseClient; animeId: number; coverUrl: string }): Promise<CachedMediaImage> {
