...
namespace Biyori.Core.IO
{
    ...
    public class IniFileParser
    {
        ...
        /// </param>
        public IniFileParser(string file, string commentDelimiter = ";")
        {
        ...
        private bool TryGetValue<T>(string section, string key, out T value)
        {
            string key2;
            ...
            var result = dictionary.TryGetValue(key2.ToLower(), out var v);
        ...
        /// <seealso cref="GetValue"/>
        public T[] GetAllValues<T>(string section, string key)
        {
            string key2, key3;
            T value;
            if (section.StartsWith("["))
            ...
            if (!dictionary.TryGetValue(key2, out var v))
                return null;
            value = (T)Convert.ChangeType(v, typeof(T));
            ...
            {
                key3 = String.Format("{0}~{1}", key2, ++index);
                if (!dictionary.TryGetValue(key3, out v))
                    break;
